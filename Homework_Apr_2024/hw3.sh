#!/bin/bash
# Bash script to calculate the MAX, MIN, MEDIAN and MEAN of the word frequencies in the
...
#Code must check if the number of unique words is even or odd. For even case median is the mean of middle two values,
...
# and the files will be added , commited and pushed to remote master using one lazy_commit command.
